...
export function registerViewEvent(
  ...
  context.subscriptions.push(
    commands.registerCommand('leek-fund.renameStockGroup', (target) => {
      ...
      window
        .showInputBox({ placeHolder: '请输入新的分组名称', value: currentName })
        .then((name) => {
          if (!name) {
            return;
          }
  ...
  context.subscriptions.push(
    commands.registerCommand('leek-fund.addStockToGroup', (target) => {
      ...
      window
        .showQuickPick(
          globalState.stockGroups.map((name, index) => ({
            label: name,
        ...
        .then((sel) => {
          ...
          LeekFundConfig.addStockToGroupCfg(sel.description, code, () => {
            stockProvider.refresh();
          });
        });
    })
  ...
  context.subscriptions.push(
    commands.registerCommand('leek-fund.addStockGroupToBar', (target) => {
      ...
      LeekFundConfig.addStockGroupToBarCfg(name, () => {
        stockProvider.refresh();
      });
    })
  );
  context.subscriptions.push(
    commands.registerCommand('leek-fund.changeStatusBarGroupItem', (groupName) => {
      const groupNames = globalState.stockGroups;
      const pickItems = groupNames
        .filter((name) => name !== groupName)
        .map((name) => ({ label: name }));
      pickItems.unshift({ label: '删除' });
      window.showQuickPick(pickItems, { placeHolder: '更换状态栏分组' }).then((res) => {
        if (!res) {
          return;
        }
        const statusBarGroups = LeekFundConfig.getConfig('leek-fund.statusBarStockGroups') || [];
        const newCfg = [...statusBarGroups];
        const index = newCfg.indexOf(groupName);
        if (res.label === '删除') {
          if (index > -1) {
            newCfg.splice(index, 1);
          }
        } else {
          if (statusBarGroups.includes(res.label)) {
            window.showWarningMessage(`分组「${res.label}」已在状态栏`);
            return;
          }
          if (index > -1) {
            newCfg[index] = res.label;
          }
        }
        LeekFundConfig.updateStatusBarStockGroupCfg(newCfg, () => {
